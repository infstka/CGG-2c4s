...
//Вычисляет новые координаты планет в СК Солнца через интервал времени dt
{
	...
	MCoords(0) = x;	MCoords(1) = y;   MCoords(2) = 1;   // Текущие координаты Луны
	fiM += wMoon * dt;                                  // Угловое положение Луны в системе кординат Земли, град
	...
	ECoords(0) = x;	ECoords(1) = y;   ECoords(2) = 1;   // Текущие координаты Земли
	P = CreateTranslate2D(x,y);
	MCoords = P * MCoords;
	                                                    // dt - интервал времени
	fiE += wEarth * dt;                                 // Угловое положение Земли в системе кординат Солнца, град
